...
void G15Applet(void)
{
	...
	if (!bAppletEnabled)
	{
		...
		if (hDLL)
		{
			...
			AddSetting(g_hWindow, IDC_CHECK9, "No Restrictions");
			AddSetting(g_hWindow, IDC_CHECK11, "Nadeo Ghosts enable");
	...
	else
	{
		...
		{
			MessageBox(0, "Failed to unload lgLcdConnect.dll", "ERROR", MB_ICONERROR);
		}
